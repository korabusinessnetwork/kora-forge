...
export default async function rotasGerador(app, { gerador, projetos, presets, design, settings }) {
  ...
  app.post('/projects/:id/plano', { config: { schemaSaida: planoSchema } }, async (request) => {
    ...
    return gerador.gerarPlano({ projeto, preset, blueprint, design: design.obter(projeto.id), workspace: settings.obter().workspace });
  });
}
