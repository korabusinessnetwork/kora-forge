...
export default async function rotasGerador(app, { gerador, projetos, presets, settings, design }) {
  ...
  app.post('/projects/:id/plano', { config: { schemaSaida: planoSchema } }, async (request) => {
    ...
    return gerador.gerarPlano({ projeto, preset, blueprint, workspace: settings.obter().workspace, tokens: design.tokensDe(projeto.id) });
  });
}
